...
public class CspglobaleImp extends Stage {
	...
	public CspglobaleImp(int editIndex, Model data) {
	...
	public void insertGcspExp() {
		...
		if ((!macherm.matches()) && (!checkBox.isSelected())) {
			...
			Pattern pExp1 = Pattern.compile(
					"([a-zA-Z]+[0-9]*[?!][a-zA-Z]+[0-9]*(?:[-]>[a-zA-Z]+[0-9]*[?!][a-zA-Z]+[0-9]*)*)[-]>\\(([a-zA-Z]+[0-9]*)(?:[?!][a-zA-Z]+[0-9]*[-]>[A-Z]+)?((?:(?:\\|~\\||\\[\\]|;|[|]{3})[a-zA-Z]+[0-9]*(?:[?!][a-zA-Z]+[0-9]*[-]>[A-Z]+)?)*)\\)");
			...
			if (checkBox.isSelected()) {
				...
				for (i = 0; (i < portName.size() && portName.get(i).cspExpression != null); i++)
					;
				if (i < portName.size())
					Alert.display("", "Please enter CSP expressions of the component's port");
				else {
					componentType.expGlobale = new Csp(" ", " ");
				...
				if (!mFormuleName.matches() && (!cspExpressionMatcher.matches()))
					...
					for (i = 0; (i < secondGroupPart.size()); i++) {
						if (port.indexOf(secondGroupPart.get(i)) == -1)
							break;
					}
					...
					if (i == secondGroupPart.size()) {
						...
						Pattern firstGroupPortNamePattern1 = Pattern
								.compile("([a-zA-Z]+[0-9]*)(?=\\|~\\||\\[\\]|;|[|]{3})");
						// first group
						if (firstPartExp != null)
							portsInCspExpression.add(firstPartExp);
						System.out.println("je suis group 1" + portsInCspExpression);
						

						// second group
						Pattern secondGroupPortNamePattern = Pattern
								.compile("(?:\\|~\\||\\[\\]|;|[|]{3})([a-zA-Z]+[0-9]*)(?=\\|~\\||\\[\\]|;|[|]{3}|$)");
						...
						for (k = 0; (k < portsInCspExpression.size()); k++) {
							if (port.indexOf(portsInCspExpression.get(k)) == -1)
								break;
						}
	...
	public String getmethodeFormule(String s) {
		String methode = s
				.replaceAll("([a-zA-Z]+[0-9]*[?!][a-zA-Z]+[0-9]*(?:[-]>[a-zA-Z]+[0-9]*[?!][a-zA-Z]+[0-9]*)*)[-]>", "");
